 ...
 *
 * Controls a dual-coil latching power relay, red/green status LEDs,
 * and implements low-power sleep/wake behavior. Operates as an I2C
 * slave at address 0x08. Supports a programmable wake timer stored
 * in EEPROM.
 *
 * ============================================================
 * HARDWARE PIN MAP (LOCKED)
 * ============================================================
 * LEDs (active-low):
 *   - GREEN  → PC2
 *   - RED    → PC3
 ...
 *
 * External Interrupts:
 *   - INT0 (Button, active-low) → PD3
 *   - INT1 (AC_OK, rising edge) → PD2
 *
 * ============================================================
 * KEY FEATURES
 * ============================================================
 * - Deferred relay pulses (never generated inside ISRs)
 * - Independent control of active-low LEDs on PORTC
 * - Power-down sleep mode
 * - Wake sources:
 *     • Button press (INT0, PD3, falling edge)
 *     • AC_OK rising edge (INT1, PD2)
 *     • Watchdog-based minute timer
 * - Software AC_OK edge detection
 * - Persistent wake timer stored in EEPROM
 ...
 * - Button pulls PD3 to GND
 * - AC_OK is HIGH when external supply is valid
 * - Relay coils require short pulses only
 *
 * ============================================================
 * LIMITATIONS
 * ============================================================
 * - Multi-byte I2C write supported only for wake timer (0x07 + 2 bytes)
 * - Read operations return status byte or extended register sequence
 * - Timer resolution ≈ 1 minute (RC oscillator drift applies)
 * - No advanced I2C fault recovery beyond peripheral reset
 *
 * ============================================================
 * BUILD & FLASH
 * ============================================================
 * Compile:
 *   avr-gcc -mmcu=atmega88pb -Os -o powercontrol.elf powercontrol.c
 *
 * Flash (Atmel-ICE ISP example):
 *   avrdude -c atmelice_isp -P usb -p atmega88pb -U flash:w:powercontrol.elf
 *
 ...
 * STACK USAGE
 * ============================================================
 * ~150 bytes worst case (estimated)
 * ISRs are shallow and no recursion is used.
 */
 ...
 * ========================================================= */
 ...
 /* Relay coils on PORTD */
 #define RELAY_SET   PD6
 #define RELAY_RESET PD5

 /* INT pins swapped */
 #define INT1_PIN    PD2   /* AC_OK  */
 #define INT0_PIN    PD3   /* Button */
 ...
 */
 ...
 {
     if (on) { PORTC &= ~(1 << RED_LED); red_state = 1; }
     else    { PORTC |=  (1 << RED_LED); red_state = 0; }
 }

 static inline void led_green(bool on)
 {
     if (on) { PORTC &= ~(1 << GREEN_LED); green_state = 1; }
     else    { PORTC |=  (1 << GREEN_LED); green_state = 0; }
 }
 ...
 */
 static void io_init(void)
 {
     ...
     PORTC |= (1 << GREEN_LED) | (1 << RED_LED);   /* LEDs OFF (active-low) */

     /* Leave PC4/PC5 alone (TWI) */

     /* --------------------------
      * PORTD — Relay + INT pins
      * -------------------------- */
     ...
     /* Pullups */
     PORTD |= (1 << INT0_PIN);   /* Button */
     PORTD |= (1 << INT1_PIN);   /* AC_OK */
