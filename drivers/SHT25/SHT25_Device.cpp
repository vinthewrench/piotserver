...
bool SHT25_Device::start(){
    ...
    if(!status){
        LOGT_ERROR("SHT25_Device(%02X) begin FAILED: %s",i2cAddr,strerror(error));
        _state = INS_INVALID;
    ...
    if(!status){
        ...
        IncidentMgr::shared()->raise(
            ...
            _resultKey_serialNo,
            nullptr,
            "SHT25 serial number read failed"
        );
    ...
    IncidentMgr::shared()->clear(
        ...
        _resultKey_serialNo,
        nullptr,
        "SHT25 serial number read succeeded"
    );
    ...
    if(_state == INS_IDLE){
        ...
        if(shouldQuery){
            ...
            if( _device.readSensor(data)){
                ...
                LOGT_ERROR("SHT25_Device(%02X) readSensor FAILED: %s", _device.getDevAddr(), strerror(errno));
