...
bool QwiicButton::hasBeenClicked(bool &state)
{
    ...
    if(_i2cPort.readByte(BUTTON_STATUS, buttonStatus.byteWrapped)){
        state =  buttonStatus.bits.hasBeenClicked;
        success = true;
    }
    return success;
...
bool QwiicButton::clearEventBits()
{
    ...
    buttonStatus.byteWrapped = 0;
    buttonStatus1.byteWrapped = 0;

    if(_i2cPort.readByte(BUTTON_STATUS, buttonStatus.byteWrapped)){

        buttonStatus.bits.isPressed = 0;
        buttonStatus.bits.hasBeenClicked = 0;
        buttonStatus.bits.eventAvailable = 0;

        success = _i2cPort.writeByte(BUTTON_STATUS, buttonStatus.byteWrapped);
