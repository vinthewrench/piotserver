...
RESTServerConnection::RESTServerConnection()
    ...
    _rURL.setCallBack([=, this]() {
        ...
        if(_rURL.method() == HTTP_OPTIONS) {
            ...
            string header =
                httpHeaderForStatusCode(STATUS_NO_CONTENT);
            ...
            header +=
                R"(Access-Control-Allow-Methods: GET, POST, PUT, DELETE,PATCH)" +
                HTTPHEADER_CRLF;

            /*
             * A 204 response ends after the blank line. It must not include
             * Content-Type, Content-Length, or a response body.
             */
        ...
        if(headers.count(HTTPHEADER_KEY_USER_AGENT)) {
            string agent =
                headers[HTTPHEADER_KEY_USER_AGENT];
        ...
        if(!mustAuthenticate || validateRequestCredentials()) {
            ...
            queueRESTCommand(
                _rURL,
                [=, this](json rp, httpStatusCodes_t code) {

                    /*
                     * Check whether the client connection was dropped while
                     * the command was queued.
                     */
                    if(!savedServer ||
                       !savedServer->isConnectionActive(savedID)) {
                        return;
                    }

                    /*
                     * HTTP 204 and 304 responses must not contain a message
                     * body. In particular, do not serialize the JSON status
                     * object generated by makeStatusJSON().
                     */
                    const bool responseHasNoBody =
                        code == STATUS_NO_CONTENT ||
                        code == STATUS_NOT_MODIFIED;

                    string body;

                    if(!responseHasNoBody && rp.size()) {
                        body = rp.dump();
                    }

                    string header =
                        httpHeaderForStatusCode(code);
                    ...
                    if(!responseHasNoBody) {
                        header +=
                            httpHeaderForContentLength(
                                body.size());
                    }

                    /*
                     * End the headers. For 204 and 304, this is also the end
                     * of the complete HTTP response.
                     */
                    header += HTTPHEADER_CRLF;
            ...
            errorReply =
                httpHeaderForStatusCode(
                    STATUS_ACCESS_DENIED);
        }

        if(errorReply.size()) {
            errorReply +=
                httpHeaderForContentLength(0) +
                HTTPHEADER_CRLF;
